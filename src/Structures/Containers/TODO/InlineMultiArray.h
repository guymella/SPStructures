...
//------------------------------------------------------------------------------
/**
    @class Oryol::InlineMultiArray
    @ingroup Core
    @brief dynamic array of Arrays , stored inline in single block of memory.
...
template<class TYPE> 
class InlineMultiArray : public iDArray<iDArray<TYPE>>{
public:
    /// default constructor
	InlineMultiArray();
    /// copy constructor (truncates to actual size)
	InlineMultiArray(const Array& rhs);
    /// move constructor (same capacity and size)
	//InlineMultiArray(Array&& rhs);
    /// initialize from initializer list
	//InlineMultiArray(std::initializer_list<TYPE> l);
    /// destructor
    ~InlineMultiArray();

    /// copy-assignment operator (truncates to actual size)
    void operator=(const iArray<iArray<TYPE>>& rhs);
    /// move-assignment operator (same capacity and size)
    void operator=(iArray<iArray<TYPE>>&& rhs);
	...
	iDArray<TYPE>& operator[](size_t index) override;
    /// read-only access to existing element
    const iDArray<TYPE>& operator[](size_t index) const override;
    /// read/write access to first element (must exists)
	iDArray<TYPE>& Front() override;
    /// read-only access to first element (must exist)
    const iDArray<TYPE>& Front() const override;
    /// read/write access to last element (must exist)
	iDArray<TYPE>& Back() override;
    /// read-only access to last element (must exist)
    const iDArray<TYPE>& Back() const override;
